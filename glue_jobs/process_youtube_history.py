...
def process_youtube_history(input_path: str, output_path: str, user_id: str) -> None:
    """
    ...
    print(
        f"Processed {len(records)} valid records (excluded {ad_count} advertisements)"
    )
    ...
    print(f"Writing {df.shape[0]} rows ({df.shape[1]} cols) to {output_s3_path}")

    # Parquet形式で保存（Snappy圧縮）
    try:
        df.to_parquet(
            output_s3_path, engine="pyarrow", compression="snappy", index=False
        )
    except Exception as e:
    ...
    stats = [
        f"input={len(data)}",
        f"ads_excluded={ad_count}",
        f"valid={len(df)}",
    ]
    if len(df) > 0:
        stats.extend(
            [
                f"range={df['watched_at'].min().date()}~{df['watched_at'].max().date()}",
                f"channels={df['channel_name'].nunique()}",
            ]
        )
...
def main():
    ...
    print(f"Starting: user={user_id}, input={input_path}, output={output_path}")
